...
final class V3DA_Admin {
    ...
    public static function handle_save(): void {
        ...
        $data = [
            ...
            'cost_level' => wp_unslash($_POST['cost_level'] ?? 0),
            'safety_rating' => wp_unslash($_POST['safety_rating'] ?? 0),
            'english_level' => sanitize_text_field(wp_unslash($_POST['english_level'] ?? '')),
            'walkability' => sanitize_text_field(wp_unslash($_POST['walkability'] ?? '')),
            'best_for' => array_map('sanitize_text_field', wp_unslash((array) ($_POST['best_for'] ?? []))),
            'avg_days' => sanitize_text_field(wp_unslash($_POST['avg_days'] ?? '')),
